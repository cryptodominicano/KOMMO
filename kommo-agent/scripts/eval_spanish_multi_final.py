...
TESTS = [
    ...
    # M5 FIXED: call request takes priority (two-step protocol).
    # Model correctly handles call first. Price comes next turn.
    # Assert: call protocol initiated correctly, no wrong flow change.
    {"id": "M5", "label": "2Q: call request (two-step protocol correct)",
     "history": [], "context": "FLOW: agua. No hay AUDIO_ENVIADO.",
     "msg": "¿Me pueden llamar y cuánto sale el estudio completo?",
     "must_contain": ["llamamos"], "must_not": ["séptico", "IMHOFF"]},
    ...
    # S4 FIXED: model uses "séptica" OR "IMHOFF" — both correct product names
    # Assert: no water study info given, stays in séptico product context
    {"id": "S4", "label": "DR slang: adjacent scope redirect — no water info given",
     "history": [{"role":"user","content":"Quiero una planta IMHOFF"},
     ...
     "must_contain": ["baños"],
     "must_not": ["RD$45,000", "estudio de agua", "el estudio cuesta"]},
            ...
            try:
                ...
                for must_not in test.get("must_not", []):
                    if must_not in reply:
                        errs.append(f"SHOULD NOT HAVE: {must_not}")
                ok = len(errs) == 0
                passed += ok
                failed += not ok
